...
public class GenericUserViewTest {
    ...
    protected static List<IArticle> addArticles(int n, int addedArticleQuantity) {
        assertTrue(n > 0 && addedArticleQuantity >= 0);
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (viewResponse.getStatus() == ResponseStatus.CREATED) {
                if (addedArticleQuantity >= 1) {
                    sellerView.setArticleQuantity(viewResponse.getData(), seller, addedArticleQuantity);
                }
    ...
    protected void setUp() {
        ...
        customer = (Customer) customerView.register(customer).getData();
    ...
    void getAll() {
        ...
        // add data
        List<IArticle> data = addArticles(100, 1);
